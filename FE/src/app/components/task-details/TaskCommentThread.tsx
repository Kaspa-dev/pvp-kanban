...
export function TaskCommentThread({
  ...
  const deleteComment = orderedComments.find((comment) => comment.id === deleteCommentId) ?? null;
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setIsSubmitting(true);
      setSubmitError("");
      await deleteTaskComment(boardId, taskId, deleteCommentId);
      onCommentsChange(orderedComments.filter((comment) => comment.id !== deleteCommentId));
      setDeleteCommentId(null);
  ...
  return (
    <>
      ...
      <DeleteCommentDialog
        ...
        authorName={deleteComment ? deleteComment.author.displayName || deleteComment.author.name : ""}
...
function DeleteCommentDialog({
  ...
  const safeAuthorName = authorName.trim() || "this user";
  ...
  return (
    <FormModalFrame
      ...
      closeAriaLabel="Close delete comment dialog"
      ...
      footer={(
        <>
          <Tooltip>
            ...
            <TooltipContent side="top" sideOffset={8}>Keep the comment</TooltipContent>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger asChild>
              <button
            ...
            <TooltipContent side="top" sideOffset={8}>Permanently delete this comment</TooltipContent>
          </Tooltip>
        </>
      )}
    >
      <div className="space-y-3">
        ...
        <p className={`text-sm leading-6 ${currentTheme.textSecondary}`}>
          This will remove {safeAuthorName}&apos;s comment from the task.
        </p>
      </div>
    </FormModalFrame>
  );
